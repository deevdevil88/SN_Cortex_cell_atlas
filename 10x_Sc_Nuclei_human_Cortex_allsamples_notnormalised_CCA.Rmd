---
title: "Sc_Nuclei_human Cortex samples, Suerat CCA"
author: "Devika"
date: "15th June 2018"
output: 
html_document:
...
```
# final vln and feature plot for each cell type based on marker genes
```{r, fig.height=16, fig.width=25}
#VlnPlot(object = Nigra.integrated, features.plot = c("MOG","ADAMTS18", "TUBB2B","STMN4","AQP4","GFAP","OLR1","TNR","CSF3R","TH","GINS3","CLDN5","XIST"), nCol = 4, do.sort = T, use.scaled=T, x.lab.rot = T)
#VlnPlot(object = Nigra.integrated, features.plot = c("MOG","ADAMTS18", "TUBB2B","STMN4","AQP4","GFAP","OLR1","TNR","CSF3R","TH","GINS3","CLDN5","XIST"), nCol = 4, do.sort = T, use.scaled=T, x.lab.rot = T, group.by="sample")
#FeaturePlot(object = Nigra.integrated, features.plot =c("MOG","ADAMTS18", "TUBB2B","STMN4","AQP4","GFAP","OLR1","TNR","CSF3R","TH","GINS3","CLDN5","XIST"), cols.use=c("grey", "red"),  reduction.use = "tsne", nCol = 4)
```
...
```
#For illustrative purposes, we can also look at the discarded cells. 
```{r, fig.height=16}
Nigra.integrated.discard <- SubsetData(Nigra.integrated.all.save, subset.name = "var.ratio.pca", accept.high = 0.5)
median(Nigra.integrated@meta.data[, "nGene"])
median(Nigra.integrated.discard@meta.data[, "nGene"])

VlnPlot(Nigra.integrated.discard, features.plot = c("MOG","ADAMTS18", "TUBB2B","STMN4","AQP4", "GFAP","OLR1","TNR","CSF3R","TH","GINS3","CLDN5","XIST"), group.by = "sample", nCol = 2)


```
# FOXP1 expression is found is some = Spiny , GOLGI, GABABPV, some monocytes as CD14 expressed,A2M,ASTROCYTE

```{r, fig.height=50}
mouseHumanGeneTable_all_Basket = mouseMarkerGenes$All$Basket %>% homologene::mouse2human()

VlnPlot(Nigra.integrated.discard, features.plot = c("SOCS2","NEUROD6","DMKN","FOXP1","CITED1","PTGER3","ATP2A3","EBF2","CASQ2","ASB4", "CALCR","HCRT","ACTN3","DAPK2","TSHZ2","FLT4","SGCG","HTR3A","GRPR","NPPC","GABRR1","GABRR2","COX6A2","AHR","CRABP1", "PTGER2","CCNB2","CD14","DSG2","CBLN3","FGF18","A2M","CD70","BAALC","AASS"), group.by = "sample", nCol = 4)
```
...
#aVERAGE OF CLUSTERS FOR Rolypoly, average using not scaled data, then log normalize and then scale and square expression values across annotations
# Average expression , no log normalisation , then scale manually and square
```{r}
#cluster.averages_scaled <- AverageExpression(object =Cortex.integrated, use.scale = T)
#cluster.averages_scaled <- as.matrix(cluster.averages_scaled)
#clusted.averages_scaled_squared <- cluster.averages_scaled^2

cluster.averages<- AverageExpression(object = Cortex.integrated, use.scale = F, return.seurat = T)
...
```
# RND2 expression in pseudo-bulk cortex data
```{r}
#FeaturePlot(object = cluster.averages, features.plot =c("RND2"), cols.use = c("grey", "red"), reduction.use = "tsne")
DotPlot(cluster.averages, genes.plot = "RND2", plot.legend = T, x.lab.rot = T, scale.by = "size", col.max = 3, col.min=-3.0)
VlnPlot(object = cluster.averages, features.plot = c("RND2"), do.sort = T, use.scaled=T, x.lab.rot = T)
DoHeatmap(object = cluster.averages, genes.use = "RND2",  
  group.label.rot = TRUE, use.scaled = T, group.cex=0,group.order = c("Astrocyte","Microglia","ODC","OPC","In1A","In1b","In1c","In3","In4A","In4B","In5","In6A","In6B","In8","Ex1","Ex2","Ex3", "Ex4","Ex5a","Ex5b","Ex6A","Ex6b","Ex8"))
...
# subset only the Astrocytes, Microglia, ODC and OPC to get matched cell types for the SN
```{r}
# Subset data based on cluster id or cell type IF
Cortex_subset <- SubsetData(object = Cortex.integrated, ident.use = c("OPC", "ODC", "Astrocyte","Microglia"), do.center = F, do.scale = F, do.clean=T)
```

# Normalize data as done before for the intial step
```{r}
Nigra_DaNeurons <- NormalizeData(object = Nigra_DaNeurons, normalization.method = "LogNormalize", scale.factor = 1e4)
```
# scale data
```{r}
Nigra_DaNeurons <- ScaleData(object = Nigra_DaNeurons,vars.to.regress = c("nUMI", "percent.mito", "percent.ribo"), model.use = "linear", do.par = T)
```

# Find variable genes for just the DaNeurons
```{r}
Nigra_DaNeurons <- FindVariableGenes(object = Nigra_DaNeurons, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.0125, x.high.cutoff = 3.0, y.cutoff = 0.5)
```
# Length of variable genes
```{r}
length(x = Nigra_DaNeurons@var.genes)
```


#RUN PCA on just the DaN cells
```{r}
Nigra_DaNeurons <- RunPCA(object = Nigra_DaNeurons, pc.genes = Nigra_DaNeurons@var.genes, do.print = T, pcs.compute = 50)
```
#PCA plots to show no correlation of PCA to different sample identity

```{r}
p<- PCAPlot(object = Nigra_DaNeurons, group.by="sample", do.return=T)
p <- p + xlab(paste0("PC1 (",signif((0.06916086*100),4),"%)"))
p <- p + ylab(paste0("PC2 (",signif(( 0.04830701*100),4),"%)"))
p

p <-PCAPlot(object = Nigra_DaNeurons, dim.1=2,dim.2=3, group.by="sample", do.return=T)
p <- p + xlab(paste0("PC2 (",signif((0.04830701*100),4),"%)"))
p <- p + ylab(paste0("PC3 (",signif(( 0.0318509*100),4),"%)"))
p



p <-PCAPlot(object = Nigra_DaNeurons, dim.1=1,dim.2=3, group.by="sample", do.return=T)
p <- p + xlab(paste0("PC1 (",signif((0.06916086*100),4),"%)"))
p <- p + ylab(paste0("PC3 (",signif(( 0.0318509*100),4),"%)"))
p

p <-PCAPlot(object = Nigra_DaNeurons, dim.1=4,dim.2=5, group.by="sample", do.return=T)
p <- p + xlab(paste0("PC4 (",signif(( 0.03062856*100),4),"%)"))
p <- p + ylab(paste0("PC5 (",signif(( 0.02844335*100),4),"%)"))
p

p <- PCAPlot(object = Nigra_DaNeurons, dim.1=5,dim.2=6, group.by="sample", do.return=T)
p <- p + xlab(paste0("PC5 (",signif(( 0.02844335*100),4),"%)"))
p <- p + ylab(paste0("PC6 (",signif(( 0.02797661*100),4),"%)"))
p
```

```{r, fig.height=20}
Nigra_DaNeurons<- ProjectPCA(object = Nigra_DaNeurons, do.print = FALSE)
VizPCA(object = Nigra_DaNeurons, pcs.use = 1:15, num.genes = 20,nCol = 4, font.size = 0.8, use.full = T)
```
...
#convert to scater object and run PCA here
```{r}
Nigra_DaN_sce <- Convert(from = Nigra_DaNeurons, to = "sce")
```

#Run PCA and find important PCS based on Sample and batch
```{r, fig.height=8, fig.width=15}
plotPCA(object = Nigra_DaN_sce, colour_by = "sample")
plotQC(Nigra_DaN_sce, type = "find-pcs", variable = "sample", ntop= 5000)
plotQC(Nigra_DaN_sce, type = "find-pcs", variable = "batch", ntop= 5000)
plotQC(Nigra_DaN_sce, type = "expl", method = "pairs", theme_size = 8)
```
```{r single-heatmap, warning=FALSE , fig.height=9}
library(RColorBrewer)
col.pal <- RColorBrewer::brewer.pal(9,name= "Blues")
PCHeatmap(object = Nigra_DaNeurons, pc.use = 1,cells.use = 195,  do.balanced = TRUE, label.columns = FALSE,col.use= col.pal)
```



```{r multi-heatmap, fig.height=30, fig.width=12, warning=FALSE}
PCHeatmap(object = Nigra_DaNeurons, pc.use = 1:29, cells.use = 195, do.balanced = TRUE, label.columns = FALSE, use.full = FALSE, col.use = col.pal, size=2)
```
```{r jackstraw, fig.height=10, fig.width=10, warning=FALSE}
...
Nigra_DaNeurons <- JackStraw(object = Nigra_DaNeurons, num.replicate = 1000, display.progress = T, do.par = T, num.cores = 1, num.pc = 35, prop.freq = 0.05)
```

```{r jsplots, fig.height=15, fig.width=12, warning=FALSE}
JackStrawPlot(object = Nigra_DaNeurons, PCs = 1:35)
```
```{r elbow_plot, fig.height=6, fig.width=10, warning=FALSE}
PCElbowPlot(object = Nigra_DaNeurons, num.pc = 10)
```
...
Nigra_DaNeurons <- FindClusters(object = Nigra_DaNeurons, reduction.type = "pca", dims.use = 1:5, resolution = 0.4, print.output = TRUE, save.SNN = TRUE, n.iter = 1000, n.start = 1000, force.recalc = T)
```

```{r tsne, fig.height=7, fig.width=7, message=FALSE, warnings=FALSE}

Nigra_DaNeurons <- SetAllIdent(Nigra_DaNeurons, id = "res.4_pca")
Nigra_DaNeurons <- RunTSNE(object = Nigra_DaNeurons, dims.use = 1:5)
#sample_Nigra_noregres <- RunTSNE(object = sample_Nigra_noregres, dims.use = 1:13)

```


```{r tsneplot, fig.height=5, fig.width=12}
...
p1 <- TSNEPlot(object = Nigra_DaNeurons, do.label = TRUE, do.return=T)
p2 <- TSNEPlot(object = Nigra_DaNeurons, group.by="Phase", do.return=T )
p3 <- TSNEPlot(object = Nigra_DaNeurons, group.by="sample", do.return=T)
p4 <- TSNEPlot(object = Nigra_DaNeurons, group.by="batch", do.return=T)
plot_grid(p1,p2)
plot_grid(p1,p3)
plot_grid(p1,p4)
#TSNEPlot(object = sample_Nigra, group.by="batches")
```
